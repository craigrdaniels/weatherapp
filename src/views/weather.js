...
const getWeather = async (coords) => {
  ...
  try {
    ...
    humidityData += '%';
    humidityElement.appendChild(createHtmlElement('span', null, ['text-sm', 'text-gray-400'], "Humidity"));
    humidityElement.appendChild(createHtmlElement('span', null, [], humidityData))
    humidityElement.classList.remove('bg-gray-300');

    // Pressure
    const pressureElement = document.getElementById('pressure');
    const pressureData = theWeather.current.pressure;
    pressureElement.appendChild(createHtmlElement('span', null, ['text-sm', 'text-gray-400'], "Pressure"));
    pressureElement.appendChild(createHtmlElement('span', null, [], pressureData));
    ...
    windspeedElement.appendChild(createHtmlElement('span', null, ['text-sm', 'text-gray-600', 'dark:text-gray-400'], "Wind Spd"));
    windspeedElement.appendChild(createHtmlElement('span', null, [], windSpeedData)); 
    ...
    winddirElement.appendChild(createHtmlElement('span', null, ['text-sm', 'text-gray-400'], "Wind Dir"));
    winddirElement.appendChild(createHtmlElement('span', null, [], windDirData)); 
...
const displaySunriseSunset = () => {
  const element = createHtmlElement('div', 'sunriseSunset', ['flex', 'justify-center', 'items-center', 'self-center', 'w-80', 'h-48', 'rounded-md', 'bg-gray-300'], '&nbsp;');

  return element;
};

const createDetailsChild = (id) => {
  const element = createHtmlElement('div', id, ['flex', 'flex-col', 'justify-evenly', 'items-center', 'text-2xl', 'w-1/4', 'h-full', 'rounded-md', 'bg-gray-300'], null);
...
const displayDetails = () => {
  const element = createHtmlElement('div', 'details', ['flex', 'justify-start', 'self-center', 'bg-gray-100', 'dark:bg-gray-700', 'gap-2', 'w-80', 'h-20', 'rounded-md'], null);
...
const displayCurrentTemperature = () => {
  const element = createHtmlElement('div', 'currentTemperature', ['flex', 'justify-center', 'items-center', 'self-center', 'font-bold', 'text-7xl', 'w-28', 'h-24', 'rounded-md', 'bg-gray-300'], '&nbsp;');

  return element;
};

const displayCityName = () => {
  const element = createHtmlElement('div', 'cityName', ['flex', 'justify-center', 'items-center', 'self-center', 'font-bold', 'text-4xl', 'w-64', 'h-10', 'rounded-md', 'bg-gray-300'], '&nbsp;');

  return element;
};

const displayWeatherIcon = () => {
  const element = createHtmlElement('div', 'weatherIcon', ['flex', 'justify-center', 'items-center', 'self-center', 'w-36', 'h-36', 'rounded-md', 'bg-gray-300'], '&nbsp;');
